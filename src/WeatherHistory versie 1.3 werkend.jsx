...
const WeatherHistory = () => {
  ...
  const [coords, setCoords] = useState(null); // Begin met null, omdat coördinaten vanuit plaatsnaam moeten worden gehaald
  ...
  }, [city, selectedDate]); // Als de stad verandert, worden de coördinaten opnieuw opgehaald
  ...
  const fetchCoordinates = async (cityName) => {
    ...
    try {
      ...
      if (data.results && data.results.length > 0) {
        ...
        setCityError(true);
        setWeatherData([]); // 🟢 Leegmaken zonder fouten
      }
    } catch (err) {
      setCityError(true);
      setWeatherData([]); // 🟢 Leegmaken zonder fouten
  ...
  const fetchWeatherData = async (date) => {
    ...
    if (!coords) {
      return; // Geen weerdata ophalen als er geen coördinaten zijn
    }
    ...
    try {
      ...
      if (data.daily && data.daily.time) {
        ...
        setWeatherData([]); // 🟢 Geen fout meer bij .length
