  ...
  useEffect(() => {
   ...
   const getColorForValue = (value, aspectType) => {
          ...
          if (aspectType === 'welzijn') {
            const intensity = 256 - Math.min(255, value * 50); // Scale max value to color range
        ...
        const datasets = dayData.map((aspect, index) => ({
          ...
          backgroundColor: aspect.data.map(item => getColorForValue(item.max, aspect.aspect_type)), // Dynamische kleur op basis van waarde
