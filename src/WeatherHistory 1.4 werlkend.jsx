...
const WeatherHistory = () => {
  ...
  const getPrecipitationSymbol = (precipitation) => {
    if (precipitation === "0.0") return "🌞"; // geen regen
    if (precipitation < 1) return "🌫️"; // mist of zeer lichte regen
    if (precipitation < 2) return "🌦️"; // lichte regen (wolkje met lichte regen)
    if (precipitation < 3) return "🌧️🌧️"; // matige regen (wolkje met regendruppel)
    if (precipitation < 5) return "☔"; // zware regen
    return "🌧️☔"; // extreem zware regen
  };
